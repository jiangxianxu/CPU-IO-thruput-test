...
static void thruput_core_statis_str_add_to_list(struct thruput_statistic
						*statis)
{
...
static struct thruput_statistic *thruput_core_statis_str_get_by_name(const char
								     *name)
{
	struct list_head *p;
	struct thruput_statistic *statistic;
...
struct thruput_statistic *thruput_core_statis_str_alloc(const char *name)
{
	...
	if (curr == &thruput_core_statistic_list) {
		statistic =
		    kmalloc(sizeof(struct thruput_statistic), GFP_KERNEL);
		if (!statistic)
...
static int thruput_core_buf_info_init(struct thruput_ctrl *ctrl, int buf_length)
{
	...
	for (i = 0; i < ctrl->task_num; i++) {
		...
		for (j = 0; j < THRUPUT_BD_NUM; j++) {
			buf_info->tx_buf[j].len = buf_length;
			buf_info->rx_buf1[j].len = buf_length;
			memcpy(buf_info->dma_buf1[j].buf, thruput_core_eth_head, THRUPUT_TEST_L2HEAD_LEN);
			buf_info->rx_buf2[j].len = buf_length;
			memcpy(buf_info->dma_buf2[j].buf, thruput_core_eth_head, THRUPUT_TEST_L2HEAD_LEN);
		}
	}
	return 0;
}
...
static void thruput_core_test_start(struct thruput_start_arg *arg)
{
	...
	ctrl_size =
	    sizeof(struct thruput_ctrl) +
	    (sizeof(struct thruput_buf_info) * task_num);
	...
	for (i = 0; i < task_num; i++) {
		...
		pass_through_task =
		    kthread_create(thruput_core_test_task, tsk_arg,
				   "%s_test", netdev->name);
		//kthread_bind(pass_through_task, 1);
...
static void thruput_core_clean_statistsc(const char *name)
{
	...
	memset(&(statistic->pkts_statistic), 0,
	       sizeof(struct thruput_pkts_statistic));
	return;
}
...
static ssize_t thruput_core_proc_write
 ...
 help:
	...
	printk
	    ("start the eth0 thruput test cmd: echo \"start [igb | ixge] eth0 [tx [lenght] | rx | pass]\" > /proc/thruput_test \n");
	printk
	    ("stop the all thruput test cmd: echo \"stop\" > /proc/thruput_test \n");
	printk
	    ("view the eth0 statistic: echo \"view eth0 statis\" > /proc/thruput_test \n");
	printk
	    ("view the eth0 statistic: echo \"clean eth0 statis\" > /proc/thruput_test \n");
	return count;
}
...
static void thruput_core_proc_initialize(void)
{
	...
	entry =
	    proc_create_data("thruput_test", 0, NULL, &thruput_core_proc_func,
			     NULL);
	if (entry == NULL) {
