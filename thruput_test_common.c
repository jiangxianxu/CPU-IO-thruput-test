...
)
{
	...
	for (i = 0; i < THTUPUT_COMMON_OPS_TBL_NUM; i++) {
    	if (thruput_common_ops_tbl[i].port_type == port_type) {
			que_num = thruput_common_ops_tbl[i].thruput_rx_que_num_get(dev);
            break;
        }
    }
...
)
{
	...
	for (i = 0; i < THTUPUT_COMMON_OPS_TBL_NUM; i++) {
    	if (thruput_common_ops_tbl[i].port_type == port_type) {
			que_num = thruput_common_ops_tbl[i].thruput_tx_que_num_get(dev);
            break;
        }
    }
...
)
{
	...
	for (i = 0; i < THTUPUT_COMMON_PORT_TYPE_MAP_TBL_NUM; i++) {
    	if (!strcmp(port_type_str, thruput_common_port_type_map_tbl[i].port_type_str)) {
			port_type = thruput_common_port_type_map_tbl[i].port_type;
            break;
        }
    }
